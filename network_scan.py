#!/usr/bin/env python3
"""
Smart Network Topology Discovery Tool (Ready-to-Run, Multi-Cred, Hardened)
Discovers Cisco network topology using CDP (with LLDP fallback) via SSH hops.
"""
...
# --- AUTH CONFIG (multi-credential support, ordered fallbacks) ---------------
# Add/adjust credential sets below. The script will try them in order.
CREDENTIAL_SETS = [
    {"username": "admin",  "password": "cisco",  "enable": ""},      # enable "" means "reuse login pw"
    {"username": "admin2", "password": "cisco2", "enable": ""},      # example secondary
    # {"username": "siteX", "password": "pwX", "enable": "enX"},     # example with distinct enable
]

# ─── DISCOVERY SETTINGS ──────────────────────────────────────────────────────
AGGREGATE_MGMT_IPS = [
    # Optional: pre-known aggregate SVI mgmt IPs to seed the queue (not including seed)
    # "10.0.100.2", "10.0.100.3",
]
TIMEOUT = 12               # general prompt timeout
MAX_READ = 65535
SSH_RETRY_ATTEMPTS = 2
SSH_RETRY_DELAY = 5
CDP_LLDP_TIMEOUT = 35      # allow more time for large outputs
# ─────────────────────────────────────────────────────────────────────────────

# Prompt detection: real device prompt line that ends with '>' or '#'
PROMPT_RE = re.compile(r"(?m)^[^\r\n#>\s][^\r\n#>]*[>#]\s?$")

# Common SSH failure snippets seen on IOS devices
FAIL_SNIPPETS = (
    "Connection refused", "Unable to connect", "Connection timed out",
    "No route to host", "Closed by remote host", "Could not resolve",
    "Host key verification failed", "% Authentication failed", "Permission denied"
)

# CDP/LLDP IP patterns
CDP_IP_PATTERNS = (
...
class NetworkDiscovery:
    def __init__(self):
        ...
        self.hostname_to_ip = {}     # global hostname->ip (informational)
    ...
    def _back_to_agg(self, shell):
        """Try to unwind nested SSH sessions back to the aggregate prompt."""
        for _ in range(4):
            try:
    ...
    def parse_lldp_neighbors(self, output):
        neighbors = []
        # split by 'Local Intf:' or 'Chassis id:'
    ...
    def _enable_candidates_for(self, ip):
        ...
        return out or [""]  # last resort: empty -> no enable
    ...
    def connect_to_aggregate(self):
        ...
        for cred in CREDENTIAL_SETS:
            try:
                ...
                # Prepare enable pw list (this cred first, then login pw as fallback)
                en_list = []
                en_list.append(cred.get("enable") or cred.get("password"))
                en_list.append(cred.get("password"))
        ...
        self.log(f"Failed to connect to aggregate with all credential sets: {last_err}", "ERROR")
        raise last_err or Exception("Unable to connect to aggregate")

    def ssh_to_device(self, shell, target_ip, attempt=1):
        self.log(f"SSH hop to {target_ip} (attempt {attempt}/{SSH_RETRY_ATTEMPTS})")

        # Order: known working for this device -> seed cred -> remaining sets
        cred_order = []
        ...
        for cred in cred_order:
            out = self.send_cmd(
                shell,
                f"ssh -l {cred['username']} {target_ip}",
                timeout=60,
                silent=True
            )

            if any(sn in out for sn in FAIL_SNIPPETS):
                self.log(f"SSH connection failed to {target_ip} (login {cred['username']})", "WARN")
                shell.send("\x03"); time.sleep(0.5); shell.send("\n"); time.sleep(0.3)
                _ = self.expect_prompt(shell, timeout=5)
                continue

            if "(yes/no)?" in out or "yes/no" in out:
                out = self.send_cmd(shell, "yes", timeout=25, silent=True)

            if re.search(r"[Pp]assword:", out):
                out = self.send_cmd(shell, cred["password"], timeout=25, silent=True)

            out += self.expect_prompt(shell, timeout=20)
            if not out:
                self.log(f"No prompt after SSH to {target_ip} (login {cred['username']})", "WARN")
                shell.send("\x03"); time.sleep(0.3); shell.send("\n"); time.sleep(0.2)
                continue
            ...
            if out.strip().endswith(">"):
                ...
                if not self._ensure_enable(shell, en_list, timeout=8):
                    self.log(f"Enable failed on {target_ip} using login {cred['username']}", "WARN")
                    self.exit_device(shell)
                    continue

            self.send_cmd(shell, "terminal length 0", timeout=5, silent=True)
            self.log(f"Successfully connected to {target_ip} with {cred['username']}")
        ...
        self.log(f"SSH to {target_ip} failed after {SSH_RETRY_ATTEMPTS} attempts (all credential sets)", "ERROR")
        return False

    def exit_device(self, shell):
        try:
            for _ in range(3):
                self.send_cmd(shell, "exit", timeout=4, silent=True)
                tail = self._drain(shell)
                if PROMPT_RE.search(tail):
                    break
        except Exception as e:
            self.log(f"Exception during exit: {e}", "DEBUG")
            self._back_to_agg(shell)
    ...
    def collect_device_info(self, mgmt_ip, skip_discovery=False):
        ...
        if mgmt_ip != AGGREGATE_ENTRY_IP:
            if not self.ssh_to_device(self.agg_shell, mgmt_ip):
                ...
                # try to ensure we’re at aggregate prompt
                try:
                    self.agg_shell.send("\n")
                    time.sleep(0.8)
                    if self.agg_shell.recv_ready():
                        buff = self.agg_shell.recv(MAX_READ).decode("utf-8", "ignore")
                        self.log(f"Current prompt tail: {buff[-100:]}", "DEBUG")
                except Exception as e:
        ...
        try:
            ...
            actual_mgmt_ip = self.get_management_ip(self.agg_shell) or mgmt_ip
            ...
            if ssh_accessible and mgmt_ip != AGGREGATE_ENTRY_IP:
...
if __name__ == "__main__":
    ...
    try:
        discovery.log_file = open(log_filename, "w", encoding="utf-8")
